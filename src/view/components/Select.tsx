  ...
  return (
    <div>
      <div className="relative">
        <label
          className={cn(
            'absolute z-10 top-[45%] -translate-y-1/2 left-3 text-gray-700 pointer-events-none',
            selectedValue &&
              ...
              <RdxSelect.Viewport className="p-2">
                {options.map((option) => (
                  <RdxSelect.Item
                    value={option.value}
                    key={option.value}
                    className="p-2 text-gray-800 text-sm outline-none data-[state=checked]:font-bold data-[highlighted]:bg-gray-50 rounded-2xl transition-colors cursor-pointer"
                  >
